...
pub const TX_MEMPOOL_DATA_PGSIZE: usize = mem::PGSIZE_2MB;
pub const TX_MEMPOOL_DATA_LEN: usize = 16384;
...
impl MempoolAllocationParams {
    ...
    pub fn new(min_items: usize, data_pgsize: usize, item_size: usize) -> Result<Self, Fail> {
        if data_pgsize != mem::PGSIZE_4KB && data_pgsize != mem::PGSIZE_2MB && data_pgsize != mem::PGSIZE_1GB {
            return Err(Fail::new(
                libc::EINVAL,
                &format!("Data pgsize provided: {} not 4KB, 2MB, or 1GB", data_pgsize),
            ));
        }
